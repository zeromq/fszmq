...
module Options =
  ...
  module SocketOption =
    ...
    let setSocketOption socket option =
      ...
      | MulticastRecovery           time        -> Socket.setOption socket (ZMQ.RECOVERY_IVL        ,int time       )
      | SendBuffer                  size        -> Socket.setOption socket (ZMQ.SNDBUF              ,int size       )
      | ReceiveBuffer               size        -> Socket.setOption socket (ZMQ.RCVBUF              ,int size       )
      | MoreMessageFramesAvailable  more        -> Socket.setOption socket (ZMQ.RCVMORE             ,more           )
      | Events                      events      -> Socket.setOption socket (ZMQ.EVENTS              ,int events     )    
      ...
      | Immediate                   true        -> Socket.setOption socket (ZMQ.IMMEDIATE           ,1              )
      | Immediate                   false       -> Socket.setOption socket (ZMQ.IMMEDIATE           ,0              )
      | Ipv6                        true        -> Socket.setOption socket (ZMQ.IPV6                ,1              )
      | Ipv6                        false       -> Socket.setOption socket (ZMQ.IPV6                ,0              )
      | KeepLastMessageInQueue      true        -> Socket.setOption socket (ZMQ.CONFLATE            ,1              )
      | KeepLastMessageInQueue      false       -> Socket.setOption socket (ZMQ.CONFLATE            ,0              )
      | LingerDelay                 delay       -> Socket.setOption socket (ZMQ.LINGER              ,int delay      )
      | MaxMessageSize              size        -> Socket.setOption socket (ZMQ.MAXMSGSIZE          ,int64 size     )
      | MaxReconnectInterval        delay       -> Socket.setOption socket (ZMQ.TCP_KEEPALIVE_INTVL ,int delay      )
      | MulticastHops               hops        -> Socket.setOption socket (ZMQ.MULTICAST_HOPS      ,int hops       )
      | ProbeRouter                 true        -> Socket.setOption socket (ZMQ.PROBE_ROUTER        ,1              )
      | ProbeRouter                 false       -> Socket.setOption socket (ZMQ.PROBE_ROUTER        ,0              )
      | ReceiveQueue                size        -> Socket.setOption socket (ZMQ.RCVHWM              ,int size       )
      ...
      | RelaxStrictAlternation      true        -> Socket.setOption socket (ZMQ.REQ_RELAXED         ,1              )
      | RelaxStrictAlternation      false       -> Socket.setOption socket (ZMQ.REQ_RELAXED         ,0              )
      | RequestCorrelation          true        -> Socket.setOption socket (ZMQ.REQ_CORRELATE       ,1              )
      | RequestCorrelation          false       -> Socket.setOption socket (ZMQ.REQ_CORRELATE       ,0              )
      | ResendDuplicateMessages     true        -> Socket.setOption socket (ZMQ.XPUB_VERBOSE        ,1              )
      | ResendDuplicateMessages     false       -> Socket.setOption socket (ZMQ.XPUB_VERBOSE        ,0              )
      | RouterMandatory             true        -> Socket.setOption socket (ZMQ.ROUTER_MANDATORY    ,1              )
      | RouterMandatory             false       -> Socket.setOption socket (ZMQ.ROUTER_MANDATORY    ,0              )
      | SendQueue                   size        -> Socket.setOption socket (ZMQ.SNDHWM              ,size           )
      | SendTimeout                 delay       -> Socket.setOption socket (ZMQ.SNDTIMEO            ,delay          )
      | TcpKeepalive                (None      )-> Socket.setOption socket (ZMQ.TCP_KEEPALIVE       ,-1             )
      | TcpKeepalive                (Some false)-> Socket.setOption socket (ZMQ.TCP_KEEPALIVE       , 0             )
      | TcpKeepalive                (Some true )-> Socket.setOption socket (ZMQ.TCP_KEEPALIVE       ,+1             )
      | TcpKeepaliveCount           count       -> Socket.setOption socket (ZMQ.TCP_KEEPALIVE_CNT   ,count          )
      | TcpKeepaliveIdle            idle        -> Socket.setOption socket (ZMQ.TCP_KEEPALIVE_IDLE  ,int idle       )
      | TcpKeepaliveInterval        delay       -> Socket.setOption socket (ZMQ.TCP_KEEPALIVE_INTVL ,int delay      )
      // PLAIN security
